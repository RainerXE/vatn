 ...
 * vatn_fs_guard.odin
 *
 * VATN FFI Demo — Odin native filesystem guard tool
 * Exported with a C ABI so the Java 25 FFM API can call it identically to C.
 *
 * Use-case: Path-level VATN guard integration — before the agent reads or
 * writes a file, call vatn_path_verdict() to get a platform-native verdict
 ...
 *
 * Build (Linux):
 *   odin build vatn_fs_guard.odin -file -build-mode:shared -out:libvatn_fs_guard.so
 *
 * Build (macOS):
 *   odin build vatn_fs_guard.odin -file -build-mode:shared -out:libvatn_fs_guard.dylib
 *
 * Build (Windows):
 *   odin build vatn_fs_guard.odin -file -build-mode:shared -out:vatn_fs_guard.dll
 *
 * Verdict constants (mirrored in VatnFsGuardTool.java):
 *   0 = ALLOW
 ...
 */
@(export, link_name="vatn_path_verdict")
vatn_path_verdict :: proc "c" (path_cstr: cstring, workspace_root_cstr: cstring) -> i32 {
 ...
 */
@(export, link_name="vatn_verdict_label")
vatn_verdict_label :: proc "c" (verdict: i32) -> cstring {
    switch verdict {
    case 0: return "ALLOW"
