...
| S6 | WebSocket | none — upgrades hostile | bad upgrade, oversized frames, flood, cross-site WS hijacking (Origin), rapid close | T5 `WsAdversarialTest` (7) | **was zero coverage** — now covered. Posture findings (positive): cross-origin upgrades are REJECTED by default on Helidon 4.5 (origin/host validation); 4MB messages handled; upgrades to non-WS routes are not honored |
...
| S8 | CORS (`vatn-plugin-cors`) | origin list = only policy | origin bypass (suffix/case/trailing slash), credentials+wildcard combo, preflight abuse, null origin | T6 `CorsAdversarialTest` (8) | **was zero tests** — now covered; wildcard+credentials combo fixed (ACAC suppressed). NOTE: `VHttpFilter` chain only guards **plugin-registered routes** — built-in `/vatn/*` routes bypass filters (by design; record when adding auth to built-ins) |
